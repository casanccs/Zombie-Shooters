...
# __Shooting Logic __
	...
	if gun_id == 1: # Pistol
		...
		if Input.is_action_just_pressed("reload"):
			var new = gunCapacity[gun_id-1] - ammo[gun_id-1]
			if new <= 0:
				gunAmmo[gun_id-1] += gunCapacity[gun_id-1]
				ammo[gun_id-1] -= gunCapacity[gun_id-1]
			else: # you don't have enough ammo for a whole clip
				gunAmmo[gun_id-1] += ammo[gun_id-1]
				ammo[gun_id-1] = 0
			GunAmmoLabel.text = str(gunAmmo[gun_id - 1])
